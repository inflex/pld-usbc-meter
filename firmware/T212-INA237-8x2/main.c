...
//	serial_send_string("\r\nPLD-UCM6M-T212\r\n");
	...
	/*
	uint8_t ch, cl;
		i2c_start(INA237|I2C_WRITE);
		i2c_write_byte(0x3E);
		i2c_start_rep( INA237|I2C_READ );
		ch = i2c_read_byte(1);
		cl = i2c_read_byte(0);
		i2c_stop();
		...
		*/
		...
		// Set the ADC to autorun on both V & A
		//
		/*
		ch &= ~(0x0F);
		ch |= 0xB0;
		i2c_start(INA237|I2C_WRITE);
		i2c_write_byte(0x00);
		i2c_start_rep( INA237|I2C_WRITE);
		i2c_write_byte(ch);
		i2c_write_byte(cl);
		i2c_stop();
		*/
	...
	while (1) {
		...
		h2s( bus_high, outs +4);
		h2s( bus_low, outs +6);
		outs[8] = 28; // 28V max range for VBUS; 20 for 20V
		outs[9] = '\n';
		outs[10] = '\0';
