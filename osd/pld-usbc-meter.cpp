...
#define OSD_MODE_NONE 0
#define OSD_MODE_20V 20
#define OSD_MODE_28V 28
			...
			do {
				...
				if (bytes_read > 0) {
					if (temp_char == '\n') {
						...
						serial_buffer[frame_size] = temp_char;
						if (frame_size_count_started) frame_size++;
					}
				}
			} while ((bytes_read > 0) && (frame_size < sizeof(serial_buffer)) && (!end_of_frame_received));

			if (frame_size < sizeof(serial_buffer)) {
				serial_buffer[frame_size] = 0;
				if (strstr(serial_buffer,"PLDUCM5M")) {
					fprintf(stderr,"Port %s selected\n", s->device);
					return PORT_OK;
				}
			}

			//65 61 64 32    39 63 39 32
			if (frame_size == 8) {
				glbs->osd_mode = OSD_MODE_20V;
				return PORT_OK;
			}

			if (frame_size == 9) {
				if (serial_buffer[8] == OSD_MODE_20V) glbs->osd_mode = OSD_MODE_20V;
				if (serial_buffer[8] == OSD_MODE_28V) glbs->osd_mode = OSD_MODE_28V;
				if (glbs->osd_mode != OSD_MODE_NONE) {
					fprintf(stderr,"Port %s selected\n", s->device);
					return PORT_OK;
				}
			}
...
int main ( int argc, char **argv ) {
	...
	double voltage_quanta = 0.004;
	double current_quanta = 0.000010 *20; // with 50mR shunt

	if (g.osd_mode == OSD_MODE_28V) {
		fprintf(stderr,"%s:%d: 28V mode selected\n", FL);
		voltage_quanta = 0.003125;
		current_quanta = 0.000005 *20; // with 50mR shunt
	}
	...
	while (!quit) {
		...
		if (g.osd_mode == OSD_MODE_20V && (bus_voltage & 0x01)) valid_data = 0;
		else valid_data = 1;

		if (g.osd_mode == OSD_MODE_20V) bus_voltage  = bus_voltage >> 3;

		current = shunt_voltage *current_quanta;
		voltage = bus_voltage *voltage_quanta;

		// Even if the current is going in the other direction, just
		// normalise the sign because it's the absolute value we care about
		//
