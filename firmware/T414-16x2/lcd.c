...
#define DDR(x) (*(&x - 1))  /* addr of data direction register of port x */
#if defined(__AVR_ATmega64__) || defined(__AVR_ATmega128__)
    /* on ATmega64/128 PINF is on port 0x00 and not 0x60 */
    #define PIN(x) ( &PORTF==&(x) ? _SFR_IO8(0x00) : (*(&x - 2)) )
#else
	#define PIN(x) (*(&x - 2)) /* addr of input register of port x */
#endif
...
//	 delay(1000);			/* wait another 4mS to be "sure" */
 ...
 //
    ...
    lcd_e_toggle();
    delay(200);           /* some displays need this additional delay */
